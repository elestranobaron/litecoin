Muscleupcoin Core version *0.15.1* is now available from:

  <https://download.muscleupcoin.org/muscleupcoin-0.15.1/>
...
A number of changes to the way Muscleupcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 toestorings per kB (0.00001 MCU). This is to prepare for dropping the minimum transaction fee to 0.1 toestorings per kB (0.0001 MCU) in the next release.
