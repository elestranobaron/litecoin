Muscleupcoin Core version 0.18.1 is now available from:

  <https://download.muscleupcoin.org/muscleupcoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/muscleupcoin-dev>
...
`/Applications/Muscleupcoin-Qt` (on Mac) or `muscleupcoind`/`muscleupcoin-qt` (on
Linux).
...
Muscleupcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Muscleupcoin Core on unsupported systems.

Muscleupcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Muscleupcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@muscleupcoin.org) the Muscleupcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 MCU),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in MCU per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/muscleupcoin-conf.md)
  about the `muscleupcoin.conf` file describes how to use it to configure
  Muscleupcoin Core.

- A new document introduces Muscleupcoin Core's BIP174 [Partially-Signed
  Muscleupcoin Transactions
  (PSBT)](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Muscleupcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Muscleupcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Muscleupcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Muscleupcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/muscleupcoin-project/muscleupcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart muscleupcoind with the `-deprecatedrpc=generate`
...
- A new `muscleupcoin-wallet` tool is now distributed alongside Muscleupcoin
...
This section describes planned changes to Muscleupcoin Core that may affect
other Muscleupcoin software and services.

- Since version 0.16.0, Muscleupcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Muscleupcoin Core 0.20 (expected about
  a year after 0.18), Muscleupcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Muscleupcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Muscleupcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Muscleupcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Muscleupcoin Core
...
- A sub-project of Muscleupcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Muscleupcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Muscleupcoin Core's own implementation, although entropy
  gathered by Muscleupcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Muscleupcoin Core a
...
- On macOS, Muscleupcoin Core now opts out of application CPU throttling
