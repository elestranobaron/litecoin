...
const char * const BITCOIN_CONF_FILENAME = "muscleupcoin.conf";
...
#else
    const char* pszModule = "muscleupcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Muscleupcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Muscleupcoin";
#else
    // Unix
    return pathRet / ".muscleupcoin";
#endif
#endif
}
